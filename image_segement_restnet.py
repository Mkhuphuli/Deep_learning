"""
x-ray image CNN segmentation adapted from  
Uddeshya Singh "CNN Segmentation ( RESNET-Depth 5 )" 
kaggle notebook by Uddeshaya Singh 
...
#######################################################
#load the target varaible such that pair is
#(patientID,location_of_pneumoninia) and for those with
#more than 1 box have more than one entry for patient
#######################################################
...
#########################################
#loading available training image names
##########################################
...
filenames = random.sample(filenames,13000)
...
##########################################
# Data exploration output for the pneumonia
# heat map, average number of Pneumonia 
...
##################################################
#class to load data data as while model is training
#since data may not fit to memory
# class takes in some filesnames, batch size and 
#other parameters and returns a random batch of numpy 
#images and numpy masks
####################################################
...
history = model.fit_generator(train_gen, validation_data=valid_gen, callbacks=[learning_rate], epochs=5, shuffle=True)

##################################################
#plotting trainging process data
####################################################
